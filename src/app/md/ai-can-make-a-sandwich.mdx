...
Chris Pine's *Learn to Program* (the Ruby book that taught a generation of developers) captures this beautifully. The computer doesn't know what a sandwich *is*. It doesn't know that bread comes in slices, that jars have lids, that knives exist. You must teach it everything, explicitly, painstakingly.
...
And now we're told: *stop that*.

Well, not entirely. But the instinct to over-specify can actively harm your results with LLMs. When you write a prompt that's too prescriptive — too step-by-step — you constrain the model's ability to leverage its latent knowledge. You force it into a narrow channel when it could be drawing from a vast reservoir.
...
Timothy Gallwey's *The Inner Game of Tennis* isn't about programming. It's about performance, learning, and the relationship between conscious instruction and unconscious execution.
...
The sandwich exercise isn't wrong. It's just... historical now. A lesson about how computers *used to be*. Worth understanding, the way it's worth understanding punch cards and assembly language. Context for where we came from.
